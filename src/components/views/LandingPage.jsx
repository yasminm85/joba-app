...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FFEFE3] text-[#2D2321] font-sans selection:bg-[#FF84BA] selection:text-white flex flex-col">
      <header className="sticky top-0 z-40 w-full bg-[#FF84BA] border-b border-black/5 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <a
              href="#about"
              className="text-xs font-bold uppercase tracking-wider text-white/90 hover:text-white transition-all">
              About
            </a>
            <a
              href="#features"
      ...
      <section
        id="about"
        ref={heroRef}
        className="bg-[#FFEFE3] py-16 md:py-24 border-b border-black/5">
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 space-y-6 text-left">
            ...
            <p className="hero-copy text-sm md:text-base font-medium text-gray-600 leading-relaxed max-w-xl">
              Turn job screenshots or raw text into a structured application
              database in seconds
            </p>
      ...
      <section
        id="features"
        ref={aboutRef}
        className="bg-[#FFDF82] py-20 text-[#2D2321] border-b border-black/5">
        <div className="max-w-6xl mx-auto px-6 space-y-12">
          <div className="about-heading text-center max-w-xl mx-auto space-y-3">
            ...
            <p className="text-xs md:text-sm font-medium text-[#2D2321]/70 leading-relaxed">
              An innovative platform to simplify job tracking and monitor your
              entire application process in one smart dashboard
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {ABOUT_FEATURES.map((feat, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="about-card bg-white/80 backdrop-blur-md p-6 rounded-3xl border border-black/5 shadow-sm space-y-3">
                  <div
                    className={`w-10 h-10 rounded-2xl flex items-center justify-center ${feat.bgClass}`}>
      ...
      <section
        id="how-it-works"
        ref={howItWorksRef}
        className="bg-[#FF84BA] py-20 text-[#2D2321] flex-1">
        <div className="max-w-6xl mx-auto px-6 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {WORK_STEPS.map((item, idx) => (
              <div
                key={idx}
                className="step-card bg-white/90 backdrop-blur-md p-6 rounded-3xl border border-black/5 shadow-sm space-y-4">
                <div
                  className={`step-number w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg ${item.numClass}`}>
                  {item.step}
                </div>
                <h3 className="text-sm font-bold uppercase tracking-wider text-gray-800">
                  {item.title}
